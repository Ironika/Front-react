...
class CartPage extends Component {
    ...
    handleSuccessPayment(payment) {
        console.log(payment);
        let order = {
            total: Number(parseFloat(Math.round((this.state.subTotal + this.state.shipping) * 100) / 100).toFixed(2)),
            user: this.state.user.id,
            orderProducts: this.state.cart
        };
        let token = window.localStorage.getItem('token');
        this.props.postOrder(token, order);
        window.localStorage.setItem('cart', JSON.stringify([]));
        this.props.editCart(this.state.cart);
        this.setState({cart: [], subTotal: this.getSubTotal(), modalIsOpen: true});
    }

    render() {
        const client = {
            sandbox:    'AaCdbEafClsnpQS-UX-wxPRlHqay9LmQK2LzKsbk7a4dw9HWoFwVX2RCU2fEhKiA13J5d_NDYYGIfBmz',
            production: 'AR0eH0dKLLCYA2xltrC8TA-LHv1pLj09Z21CZX1dl0ZJZ8XppiAT8Byung1zkzfqQMZgfPGFeASpImlK',
        };
        return (
            <main className="cart" id="Cart">
                                        ...
                                        <div className="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
                                            ...
                                            </div>
                                            {
                                                this.props.location.pathname == '/cart' ?
                                                    ...
                                                    <div className="flex-c-m stext-101" onClick={this.handleClickCheckout.bind(this)}>
                                                        <PaypalExpressBtn env={'sandbox'} client={client} currency={'USD'} onSuccess={this.handleSuccessPayment.bind(this)} total={ Number(parseFloat(Math.round((this.state.subTotal + this.state.shipping) * 100) / 100).toFixed(2)) } />
                                                    </div>
                                            }
                                        </div>
                                    </div>
                                </div> : 
                                <h4 className="cart-empty">Cart Empty</h4>
                            }
                        </div>
                    </section>
                </LazyLoad>
                    ...
                    <div className="container">
                        <div className="bg0 p-lr-15-lg how-pos3-parent">
                            ...
                            <div className="modal-product">
                                ...
                                <p>Your payment have been registered !</p>
                            </div>
                        </div>
                    </div>
                </Modal>
...
const mapDispatchToProps = dispatch => (
    bindActionCreators({ editCart, postOrder }, dispatch)
);
